...
export default function MessagingUI() {
  ...
  return (
    <div className="rounded-2xl flex h-full overflow-hidden bg-gradient-to-br from-teal-100 via-cyan-100 to-white shadow-lg">
      {/* Sidebar */}
      <div className="flex-shrink-0 w-1/3 bg-white bg-opacity-90 py-5 border- border-cyan-200 shadow-inner overflow-y-auto">
        <div className="flex px-5 items-center justify-between mb-5">
          <h2 className="text-2xl font-semibold text-teal-800 tracking-wide">
            Friends
          </h2>
          <button
          ...
          {friends.length === 0 ? (
            ...
            friends.map((friend) => (
              <li
                ...
                className={`flex ml-5 items-center cursor-pointer py-2 px-3 rounded-l-lg transition-colors
              ${
      ...
      <div className="flex flex-col flex-1 bg-white bg-opacity-90 pb-5">
        <h2 className="text-2xl font-semibold text-teal-900 p-3  select-none">
          {friends.length && activeFriend
            ? activeFriend.name
            : "Select a friend to start chatting"}
        </h2>
        <div className="flex-1 rounded-2xl bg-cyan-100 overflow-y-auto pl-2 pt-5 pr-4 space-y-1">
          {friends.length >0 && activeFriend ? (
            messages
              ...
              .map((msg, idx) => (
                ...
                >
                  <div
                    className={`px-5 py-2 rounded-3xl max-w-xs md:max-w-lg shadow- ${
                      msg.sender === auth.currentUser.uid
                  ...
                  >
                    <p className="text-base font-medium">{msg.messages}</p>
            ...
            <p className="text-gray-500 italic">No meffssages yet</p>
          )}
          <div ref={messagesEndRef} />
        </div>

        {/* Input */}
        <div className="flex mt-2 bg-white rounded-full border border-cyan-300 shadow-sm p-2">
          <input
            type="text"
            value={inputText}
